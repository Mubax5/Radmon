...
def test_env_uses_neutral_production_endpoints_and_archive_settings():
    text = (ROOT / ".env.example").read_text(encoding="utf-8")
    assert "RADMON_CENTRAL_HOST=192.168.1.2" in text
    assert "RADMON_LAN_SOURCES=source-a@IP-A;source-b@IP-B;source-c@IP-C" in text
    assert "192.168.1.50" not in text
    assert "192.168.1.52" not in text
    assert "192.168.1.38" not in text
    for name in (
