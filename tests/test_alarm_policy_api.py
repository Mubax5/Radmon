...
def fixture(tmp_path):
    ...
    security.create_user("admin", "Administrator", Role.ADMINISTRATOR, "Password123!", "2468")
...
def suppression_payload(pin: str, pic: str = "Budi") -> dict[str, object]:
    return {
        "pin": pin,
...
def response_payload(pin: str, pic: str = "Budi") -> dict[str, str]:
    return {"pin": pin, "action": "Confirm", "pic": pic, "reason": "checked"}


def create_alarm(policy: AlarmPolicyService, serid: int = 5201):
    return policy.evaluate_live({
        "serid": serid,
...
def test_unauthenticated_policy_mutations_return_401(tmp_path):
    client, _ = fixture(tmp_path)
    assert client.post(
        "/api/v1/control/suppressions/5201",
        json=suppression_payload("1357"),
    ).status_code == 401
    assert client.post(
        "/api/v1/control/alarm-events/missing/response",
        json=response_payload("1357"),
    ).status_code == 401
...
def test_viewer_cannot_mutate_operator_alarm_workflow(tmp_path):
    client, policy = fixture(tmp_path)
    snap = create_alarm(policy)
    login(client, "viewer")
    assert client.post(
        "/api/v1/control/suppressions/5201",
        json=suppression_payload("9999", "Viewer"),
    ).status_code == 403
    assert client.post(
        f"/api/v1/control/alarm-events/{snap['active_event_id']}/response",
        json=response_payload("9999", "Viewer"),
    ).status_code == 403
...
def test_administrator_inherits_operator_alarm_mutations(tmp_path):
    client, policy = fixture(tmp_path)
    snap = create_alarm(policy)
    login(client, "admin")
    response = client.post(
        f"/api/v1/control/alarm-events/{snap['active_event_id']}/response",
        json=response_payload("2468", "Administrator"),
    )
    assert response.status_code == 200

    suppression = client.post(
        "/api/v1/control/suppressions/5202",
        json=suppression_payload("2468", "Administrator"),
    )
