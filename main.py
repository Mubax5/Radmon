...
from radmon.desktop_app import run_admin_ui
...
def _run_legacy_detector_or_dummy(source: str) -> int:
...
def main() -> int:
    ...
    if args.source == "lan":
        ...
        try:
            ...
            QMessageBox.critical(None, "Security tidak siap", str(exc))
            return 4
        return run_admin_ui(settings, services, archive_catalog, log_path)

    # Transitional detector/dummy path; Task 12 removes this top-level launcher.
    if args.source != "lan":
        return _run_legacy_detector_or_dummy(args.source)
    return 0
