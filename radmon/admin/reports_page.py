...
from PySide6.QtWidgets import QCheckBox, QDateTimeEdit, QFileDialog, QHBoxLayout, QLabel, QMessageBox, QPushButton, QTableWidget, QTableWidgetItem, QVBoxLayout, QWidget
...
class ReportsPage(QWidget):
    def __init__(self, report_service, settings, parent=None):
        ...
        for widget in (self.start, self.end): widget.setCalendarPopup(True); widget.setDisplayFormat("yyyy-MM-dd HH:mm:ss")
        self.live = QCheckBox("Live"); self.live.setChecked(True)
        ...
        controls = QHBoxLayout(); controls.addWidget(QLabel("From")); controls.addWidget(self.start); controls.addWidget(QLabel("To")); controls.addWidget(self.end); controls.addWidget(self.live); controls.addWidget(export); controls.addWidget(open_pdf); controls.addStretch(1)
    ...
    def refresh_live(self) -> None:
        if self.live.isChecked(): self.end.setDateTime(QDateTime.currentDateTime())
        try:
            ...
            for r, row in enumerate(rows):
                for c, value in enumerate([row.get("dtom"), row.get("doserate"), row.get("stat")]):
            ...
            self.last_error = f"Report read error: {exc}"
