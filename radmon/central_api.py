...
""", (station["serid"], station["name"], station["location"], station["maxidlemin"], station["warnlevel"], station["alarmlevel"], station["unit"], source_name[:50], f"Synced from {source_name}"[:255]))
...
""", (item["serid"], item["dtom"], item["doserate"], item.get("dose", 0.0), item["previnterval"], item["stat"]))
                    if getattr(cursor, "rowcount", 0) == 1:
                        ...
                        if recent_time is None or item["dtom"] >= recent_time:
                            ...
                            measurement = Measurement(serid=int(item["serid"]), measured_at=item["dtom"], dose_rate=float(item["doserate"]), previnterval=int(item["previnterval"]), stat=int(item["stat"]))
                            upsert_recent(cursor, measurement, dose=float(item.get("dose") or 0.0), previous_time=previous_time, previous_rate=float(previous_rate) if previous_rate is not None else None, previous_dose=float(previous_dose) if previous_dose is not None else None, interval=int(item["previnterval"]))
            connection.commit()
            return inserted
        except Exception:
            connection.rollback()
    ...
    def latest(self, serid: int) -> dict[str, Any] | None:
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT serid, dtom, doserate, dose, previnterval, stat FROM measurement WHERE serid = ? ORDER BY dtom DESC LIMIT 1", (serid,))
                row = cursor.fetchone()
            if row is None:
                return None
