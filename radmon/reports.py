...
class ReportService:
    ...
    def summary(self, start: datetime, end: datetime) -> ReportSummary:
        ...
        return ReportSummary(first_measurement=min(times) if times else None, last_measurement=max(times) if times else None, minimum=min(values) if values else None, average=fmean(values) if values else None, maximum=max(values) if values else None, sample_count=len(values), approximate_dose=approximate_dose(usable))
    ...
    def export_pdf(self, start: datetime, end: datetime, destination: Path | str) -> Path:
        ...
        doc = SimpleDocTemplate(str(path), pagesize=landscape(A4), leftMargin=12*mm, rightMargin=12*mm, topMargin=12*mm, bottomMargin=12*mm, title=f"Radmon Report - {station.room}", author="Radmon DPFK Python")
        ...
        for row in alarms[:500]: alarm_data.append([str(row.get("alarmid","-")),self._dt(row.get("dtom")),str(row.get("type","-")),str(row.get("msg") or "-")])
