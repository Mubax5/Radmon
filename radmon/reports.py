...
def approximate_dose(rows: list[dict[str, Any]]) -> float:
    """Integrate dose-rate (µSv/h) over elapsed hours using trapezoids."""
    points = [
...
class ReportService:
    ...
    def export_csv(self, start: datetime, end: datetime, destination: Path | str) -> Path:
        ...
        with path.open("w", newline="", encoding="utf-8") as handle:
            ...
            for row in rows:
                ...
                if isinstance(serializable.get("dtom"), datetime):
                    serializable["dtom"] = serializable["dtom"].strftime("%Y-%m-%d %H:%M:%S")
    ...
    def export_pdf(self, start: datetime, end: datetime, destination: Path | str) -> Path:
        ...
        alarms = self.repository.alarm_history(start, end, serid=self.settings.serid, limit=5000)
        ...
        doc = SimpleDocTemplate(
            ...
            author="Radmon DPFK Python",
        )
        story: list[Any] = [
            Paragraph("LAPORAN PEMANTAUAN RADIASI", styles["Title"]),
            Paragraph(f"{station.room} · {station.location} · ID {station.serid}", styles["Heading2"]),
            Paragraph(f"Periode: {start:%Y-%m-%d %H:%M:%S} s/d {end:%Y-%m-%d %H:%M:%S}", styles["BodyText"]),
            Paragraph(f"Threshold: Alert {station.warnlevel:g} µSv/h · Alarm {station.alarmlevel:g} µSv/h", styles["BodyText"]),
            Spacer(1, 6 * mm),
        ]
        summary_data = [
            ["First", "Last", "Min (µSv/h)", "Average (µSv/h)", "Max (µSv/h)", "Samples", "Approx. Dose (µSv)"],
            [
        ...
        story.extend([Paragraph("Summary", styles["Heading2"]), summary_table, Spacer(1, 6 * mm)])
        ...
        for index, row in enumerate(rows[:1000], start=1):
            measurement_data.append([
                ...
                self._fmt(float(row["doserate"])) if row.get("doserate") is not None else "-",
                str(row.get("previnterval", "-")),
                str(row.get("stat", "-")),
            ])
        if len(measurement_data) == 1:
            measurement_data.append(["-", "No data", "-", "-", "-"])
        measurements = Table(measurement_data, repeatRows=1, colWidths=[12*mm, 50*mm, 42*mm, 30*mm, 20*mm])
        measurements.setStyle(self._table_style())
        story.extend([Paragraph("Measurement data (maks. 1000 row pada PDF)", styles["Heading2"]), measurements, Spacer(1, 6 * mm)])
        ...
        story.extend([Paragraph("Alarm history", styles["Heading2"]), alarm_table, Spacer(1, 4 * mm)])
        story.append(Paragraph(f"Generated: {datetime.now():%Y-%m-%d %H:%M:%S}", styles["BodyText"]))
        doc.build(story)
        return path
